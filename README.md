...
- **计算下推**：在本地利用 Polars 引擎完成 SQL 过滤、聚合，仅输出结果。
- **惰性求值**：支持处理远超内存大小的超大规模数据集。
- **多格式支持**：原生支持 CSV, JSON, Parquet, Arrow, 日志文件等。
...
基于 9.5GB 测试数据集的实际测试结果（2026-02-25）：
...
- 标准搜索: p50=0.0ms, p95=0.2ms, p99=0.2ms
- 前缀搜索: p50=0.0ms, p99=0.0ms
- 模糊搜索: p50=0.0ms, p99=0.3ms

### 性能优势

| 对比项 | 传统工具 (grep/rg) | XORE (Agent-Native) | 优势 |
|-------|---------|------|------|
| **Token 效率** | 原始文本搬运 | **计算下推/结构化摘要** | **节省 90%+ Token** |
| **全文搜索** | grep (线性扫描) | 索引加速 | 1000x+ |
| **文件查找** | find (目录遍历) | 并行扫描 | 10x+ |
| **正则搜索** | ripgrep (无索引) | 索引后搜索 | 100x+ |
